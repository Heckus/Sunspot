...
def main_loop(hw_manager, cam_manager):
    ...
    # Get initial primary camera FPS for loop timing (can use actual reported FPS here)
    try:
    ...
    while not shutdown_event.is_set():
        ...
        if requested_resolution_index is not None:
            ...
            if was_recording:
                ...
                cam_manager.stop_recording() # Handles video thread, audio stop, mux
                time.sleep(0.5)
        ...
        # This now captures, combines (if needed), updates stream frame,
        # AND puts the raw frame into the recording queue if active.
        combined_frame_for_stream = cam_manager.capture_and_combine_frames()

        # Check if capture itself failed (frame0 was None)
        ...
        if should_be_recording and not is_currently_recording:
            ...
            if not cam_manager.start_recording(): # Starts video thread and audio
            ...
            cam_manager.stop_recording() # Stops video thread, audio, muxes
        ...
        # --- Loop Delay ---
        # Base delay on primary camera's current FPS for capture responsiveness
        loop_duration = time.monotonic() - loop_start_time
        if current_cam0_fps > 0:
            target_loop_time = 1.0 / current_cam0_fps
        else:
            target_loop_time = 1.0 / 30.0 # Fallback
        ...
        if current_time - last_loop_log_time > 10.0:
             # Add queue size info if recording
             q_info = ""
             if cam_manager.is_recording and cam_manager.recording_frame_queue:
                  q_info = f", RecQ: {cam_manager.recording_frame_queue.qsize()}"
             logging.debug(f"MainLoop duration: {loop_duration:.4f}s, Sleep: {sleep_time:.4f}s (Target Cap FPS: {current_cam0_fps:.2f}{q_info})")
