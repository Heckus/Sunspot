...
def index():
    ...
    global undistort_active # Added undistort state
    ...
    with config_lock:
        ...
        undistort_state_initial = undistort_active # Get initial undistort state
    ...
    # Define HTML Template (Standard String)
    html_template = """
    <!DOCTYPE html>
    <html>
    <head>
        ...
        <style>
            ...
            #status, #rec-status, #resolution, #battery-level, #awb-mode-status, #ae-mode-status, #metering-mode-status, #nr-mode-status, #undistort-status { color: #0056b3; font-weight: normal;}
            ...
            img#stream { display: block; margin: 15px auto; border: 1px solid black; /* Use original width/height for now */ background-color: #ddd; } 
    ...
    <body>
        <div class="container">
            <h1>Pi Camera Stream & Record</h1>
             ...
             <div class="main-controls">
                 ...
                 <button onclick="toggleUndistort()" id="btn-undistort" class="undistort-inactive" title="Toggle Fisheye Undistortion (Performance Cost!)">Undistort OFF</button>
        ...
        <script>
            ...
            const btnUndistort = document.getElementById('btn-undistort'); // Undistort button
            const undistortStatusElement = document.getElementById('undistort-status'); // Undistort status span
            ...
            let isTogglingUndistort = false; // Flag for undistort toggle
            ...
            function updateRecordButtonState() { /* (Same as before) */
                if (currentDigitalRecordState) {
                    btnRecord.textContent = "Stop Rec (Web)"; btnRecord.classList.remove('recording-inactive'); btnRecord.classList.add('recording-active');
                } else {
                    btnRecord.textContent = "Start Rec (Web)"; btnRecord.classList.add('recording-inactive'); btnRecord.classList.remove('recording-active');
                }
            }

            function updateUndistortButtonState(isActive) { /* (New) */
                 if (isActive) {
                     btnUndistort.textContent = "Undistort ON"; btnUndistort.classList.remove('undistort-inactive'); btnUndistort.classList.add('undistort-active');
                     undistortStatusElement.textContent = "ON"; undistortStatusElement.classList.add('active');
                 } else {
                     btnUndistort.textContent = "Undistort OFF"; btnUndistort.classList.add('undistort-inactive'); btnUndistort.classList.remove('undistort-active');
                     undistortStatusElement.textContent = "OFF"; undistortStatusElement.classList.remove('active');
                 }
            }
                ...
                fetch('/status')
                    ...
                    .then(data => {
                        ...
                        if (data.resolution && resolutionElement.textContent !== data.resolution) {
                            ...
                            // Re-apply width/height if they exist (using the reverted HTML)
                        ...
                        // Update Undistort State 
                        if (typeof data.undistort_active === 'boolean') {
                             updateUndistortButtonState(data.undistort_active);
                        }
                    ...
                    .catch(err => { 
                        ...
                        awbStatusElement.textContent = "Err"; aeStatusElement.textContent = "Err"; meteringStatusElement.textContent = "Err"; nrStatusElement.textContent = "Err"; undistortStatusElement.textContent = "Err";
                    });
            }

            function updateControlUI(controlKey, newValue, statusEl, controlEl, valueSpanEl = null) { /* (Same as before) */
                 if (newValue === undefined || newValue === null) return; 
                 let currentUIValue = controlEl ? controlEl.value : null;
                 let formattedNewValue = newValue;
                 if (valueSpanEl) { 
                     formattedNewValue = parseFloat(newValue).toFixed(1); currentUIValue = parseFloat(currentUIValue).toFixed(1);
                     if (valueSpanEl.textContent !== formattedNewValue) { valueSpanEl.textContent = formattedNewValue; }
                 }
                 if (statusEl && statusEl.textContent !== newValue.toString()) { statusEl.textContent = newValue.toString(); }
            ...
            function disableControls(poweringDown = false) { /* (Updated) */
                [btnUp, btnDown, btnRecord, btnUndistort, btnPowerdown, // Added btnUndistort
                ...
                ].forEach(el => { if(el) el.disabled = true; }); // Add check for null element
                if(poweringDown) { document.body.style.opacity = '0.7'; }
            }

            function enableControls() { /* (Updated) */
                 if (!isPoweringDown) {
                    [btnUp, btnDown, btnRecord, btnUndistort, btnPowerdown, // Added btnUndistort
                     awbSelectElement, aeSelectElement, meteringSelectElement, nrSelectElement,
                     brightnessSlider, contrastSlider, saturationSlider, sharpnessSlider
                    ].forEach(el => { if(el) el.disabled = false; }); // Add check for null element
                    document.body.style.opacity = '1';
                 }
            }

            // --- Action Functions ---
            function changeResolution(direction) { /* (Updated Check) */
                if (isChangingResolution || isTogglingRecording || isChangingControl || isTogglingUndistort || isPoweringDown) return;
                ...
                const resolutionTimeoutId = setTimeout(() => { /* ... */ }, 8000); 
                fetch(`/set_resolution/${direction}`, { method: 'POST' })
                    ...
                    .then(({ status, body }) => {
                        if (status === 200 && body.success) {
                            ...
                            streamImage.setAttribute('width', w); // Keep applying width/height as we reverted HTML
                            streamImage.setAttribute('height', h);
                            console.log("Resolution change request successful, forcing stream reload...");
                            streamImage.src = videoFeedUrlBase + "?" + Date.now(); 
                        } else { /* Handle failure */ clearTimeout(resolutionTimeoutId); cleanupResolutionChange(false); }
                    })
                    .catch(err => { /* Handle error */ clearTimeout(resolutionTimeoutId); cleanupResolutionChange(false); });
            }

            function toggleRecording() { /* (Updated Check) */
                 if (isChangingResolution || isTogglingRecording || isChangingControl || isTogglingUndistort || isPoweringDown) return;
                 /* ... (rest of function same) ... */ 
            }
            function changeCameraControl(controlName, controlValue) { /* (Updated Check) */
                 if (isChangingResolution || isTogglingRecording || isChangingControl || isTogglingUndistort || isPoweringDown) return;
                 /* ... (rest of function same) ... */ 
            }
            
            function toggleUndistort() { /* (New) */
                if (isChangingResolution || isTogglingRecording || isChangingControl || isTogglingUndistort || isPoweringDown) return;
                isTogglingUndistort = true; 
                disableControls(); 
                statusElement.textContent = 'Toggling undistortion...'; 
                errorElement.textContent = ''; errorElement.style.display = 'none';
                fetch('/toggle_undistort', { method: 'POST' })
                    .then(response => response.json().then(data => ({ status: response.status, body: data }))) 
                    .then(({ status, body }) => {
                        if (body.success) {
                            updateUndistortButtonState(body.undistort_active);
                            statusElement.textContent = body.message || `Undistortion ${body.undistort_active ? 'enabled' : 'disabled'}.`;
                            setTimeout(updateStatus, 1500); 
                        } else {
                            errorElement.textContent = `Error toggling undistortion: ${body.message || 'Unknown error.'}`; 
                            errorElement.style.display = 'block'; 
                            statusElement.textContent = 'Undistort toggle failed.';
                            setTimeout(updateStatus, 1000); 
                        }
                    })
                    .catch(err => {
                         console.error("Error toggling undistortion:", err); 
                         errorElement.textContent = `Network error toggling undistortion: ${err.message}`; 
                         errorElement.style.display = 'block'; 
                         statusElement.textContent = 'Undistort toggle failed (Network).'; 
                         setTimeout(updateStatus, 1000); 
                    })
                    .finally(() => { 
                        isTogglingUndistort = false; 
                        enableControls(); 
                    });
            }

            function updateSliderValue(sliderId, value) { /* (Same as before) */ }
            function powerDown() { /* (Updated Check) */
                 if (isChangingResolution || isTogglingRecording || isChangingControl || isTogglingUndistort || isPoweringDown) return;
                 /* ... (rest of function same) ... */ 
            }
            ...
            function handleStreamLoad() { /* (Same as before - only clears error timeout) */ }

            // --- Initialization ---
            document.addEventListener('DOMContentLoaded', () => { /* (Use simple version without try/catch for now) */
                console.log("DOMContentLoaded event fired.");
                updateRecordButtonState();
                // Fetch initial status which includes undistort state
                updateStatus(); 
                ...
                if (!statusUpdateInterval) {
                 ...
                 // Enable controls after initial setup attempt
    ...
    return render_template_string(html_template,
                                   ...
                                   digital_rec_state_initial=digital_rec_state_initial, batt_text_initial=batt_text_initial,
                                   undistort_state_initial=undistort_state_initial, # Pass initial state
