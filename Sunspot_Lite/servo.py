...
# Use LGPIOFactory for Pi 5 / Bookworm compatibility
...
# --- Configuration ---
# Using GPIO 12 as it supports Hardware PWM on Raspberry Pi 5
# Hardware PWM generally provides a more stable signal than software PWM.
# Using LGPIOFactory which uses the lgpio library/daemon, recommended for Pi 5.
SERVO_PIN = 12
...
try:
    # Using LGPIOFactory is recommended for Pi 5 as it uses the lgpio library
    # and lgd daemon, which interfaces with the modern kernel gpiochip subsystem.
    # This should provide stable PWM, including hardware PWM on capable pins.
    # Make sure lgd daemon is running: sudo systemctl start lgd
    factory = LGPIOFactory()
    servo = Servo(SERVO_PIN, pin_factory=factory) # min_pulse_width=min_pulse, max_pulse_width=max_pulse)
    print(f"Servo initialized on GPIO {SERVO_PIN} (Hardware PWM capable pin) using lgpio (LGPIOFactory).")

except Exception as e:
    print(f"Error initializing servo with lgpio (LGPIOFactory): {e}")
    print("Have you installed python3-lgpio?")
    print("Is the lgd service running? Try: sudo systemctl start lgd")
    ...
    try:
        ...
        print("Note: lgpio on a hardware PWM pin is recommended for best results on Pi 5.")
    except Exception as fallback_e:
        print(f"Fallback failed: {fallback_e}")
        print("Exiting. Please check GPIO setup, permissions, and libraries (python3-gpiozero, python3-lgpio).")
...
def create_gui():
    ...
    root = tk.Tk()
    root.title("Raspberry Pi Servo Control (HW PWM via lgpio)")
    ...
    # Servo Controls
    label = tk.Label(main_frame, text=f"Servo Control (GPIO {SERVO_PIN} - HW PWM via lgpio)")
    label.pack(pady=(0, 5))
...
def cleanup_gpio(signum=None, frame=None):
    ...
    if root:
        try:
            ...
            # Ignore error if window already closed (common on rapid exit)
    ...
    # No factory.close() method needed for LGPIOFactory as per gpiozero docs
    # if factory:
    #     try:
    #         # LGPIOFactory might not have/need an explicit close
    #         # factory.close()
...
if __name__ == "__main__":
    # Check if lgd service needs to be started
    print("Make sure the lgpio daemon (lgd) is running for best results:")
    print(" sudo systemctl start lgd")
    print("------------------------------------")
