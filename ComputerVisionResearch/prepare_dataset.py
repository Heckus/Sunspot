# prepare_dataset_multidigit.py (Revised)
...
# Configuration
# =============================================================================
# --- IMPORTANT: PLEASE VERIFY THIS PATH ---
# This should be the absolute path to your dataset folder.
# Example: '/home/hecke/Code/SJN-210k'
...
# --- SAMPLE LIMIT FOR DEBUGGING ---
# Set this to a small number (e.g., 1000) to test without running out of memory.
...
def process_data(label_file_path, image_base_dir, limit=None):
    """
    Processes all images and labels defined in a label file.
    Includes a limit for debugging memory issues.
    """
    ...
    if limit:
        lines = lines[:limit] # Limit the number of samples to process
    ...
    for i, line in enumerate(lines):
        ...
        if i < 5: # Print the first 5 attempted paths
            print(f"  --> Attempting to load image: {img_path}")

        if not os.path.exists(img_path):
            if i < 5: # Only print warnings for the first few to avoid spam
...
(train_full_imgs, train_num_imgs, train_bbox, 
 train_labels, train_filenames) = process_data(train_label_path, train_image_dir, limit=SAMPLE_LIMIT)

print("[INFO] Processing testing data...")
(test_full_imgs, test_num_imgs, test_bbox, 
 test_labels, test_filenames) = process_data(test_label_path, test_image_dir, limit=SAMPLE_LIMIT)

# (The rest of the script for saving pickles is the same...)
# ...
# The code below is identical to the previous version
print(f"[INFO] Processed {len(train_full_imgs)} training samples and {len(test_full_imgs)} testing samples.")

if not train_full_imgs or not test_full_imgs:
    print("[ERROR] No samples were processed. Please check the DATASET_BASE_PATH and ensure image files exist.")
else:
